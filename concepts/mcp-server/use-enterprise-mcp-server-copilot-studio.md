...
- A Microsoft Entra tenant (not MSIT; custom connectors in MSIT tenants are blocked by DLP policy enforcement unless the connector is certified as first party).
...
- Complete the MCP Server provisioning steps in [Get started with the Microsoft MCP Server for Enterprise](/graph/mcp-server/get-started), including granting admin consent for the MCP Server API permissions on your app registration.
- A [client app registration](/entra/identity-platform/quickstart-register-app) in Microsoft Entra. You need the following from your app registration:
  ...
  - **Client secret** (from **Certificates & secrets**). Alternatively, if client secrets aren't permitted by policy, use federated credentials by creating a managed identity and adding it to your **Certificates & secrets** tab.
...
1. Fill in the following required fields:
    ...
    | Field | Value |
    |---|---|
    | **Server name** | `Microsoft MCP Server for Enterprise` |
...
1. Fill in the following OAuth fields:
    ...
    | **Client secret** | The client secret value of your client app registration. |
...
1. Select **+ Add a platform** > **Web**. If the Web platform already exists, select **+ Add URI** instead.
...
1. Find your connector (named **Microsoft MCP Server for Enterprise**) in the list.
...
Before you connect the tool to your agent, verify that the connector can establish a connection.
...
1. Find your **Microsoft MCP Server for Enterprise** connector and select it to add it to the agent.
...
> [!NOTE]
> The Microsoft MCP Server for Enterprise currently supports **read-only** operations. Write actions aren't allowed.
