...
| enforcementResultStatus          | enforcementResultStatus                                                                         | Indicates the enforcement outcome reported by the enforcement plane after a DLP policy match. The possible values are: `success`, `missingOrInvalidConfiguration`, `userOverride`, `agentFailure`, `enforcementTimeout`, `oSOverride`, `processNonExistent`, `other`. For more information, see [Understand enforcement result status in Microsoft Purview](/graph/security-enforcement-result-status.md).                                       |
