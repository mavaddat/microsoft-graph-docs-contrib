...
Represents an established governance relationship between a governing tenant (typically a managed service provider or partner organization) and a governed tenant (customer organization). A governance relationship is created when a [governanceRequest](tenantgovernanceservices-governancerequest.md) is accepted by the governed tenant.
