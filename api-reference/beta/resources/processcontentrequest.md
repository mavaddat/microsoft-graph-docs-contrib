...
|contentEntries|[microsoft.graph.processContentMetadataBase](../resources/processcontentmetadatabase.md) collection|A collection of content entries to be processed. Each entry contains the content itself and its metadata. Use [conversation metadata](./processconversationmetadata.md) for content like prompts and responses, [file metadata](./processfilemetadata.md) for files, and [content activity metadata](./contentactivitymetadata.md) for enforcement result status entries. Required.|
