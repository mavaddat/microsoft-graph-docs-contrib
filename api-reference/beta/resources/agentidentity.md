...
|inheritedAppRoleAssignments|[appRoleAssignment](../resources/approleassignment.md) collection|Read-only collection of app role assignments inherited from the parent Agent Identity Blueprint Service Principal, computed at runtime. The `principalId` on each assignment is set to this agent identity's service principal ID. Read-only. Nullable. No OData query parameters are supported (`$filter`, `$top`, `$skip`, `$count`, `$orderBy`, `$expand`, `$select`).|
|inheritedOauth2PermissionGrants|[oAuth2PermissionGrant](../resources/oauth2permissiongrant.md) collection|Read-only collection of OAuth2 permission grants inherited from the parent Agent Identity Blueprint Service Principal, computed at runtime. The `clientId` on each grant is set to this agent identity's service principal ID. Only grants of `consentType` **AllPrincipals** are returned. Read-only. Nullable. No OData query parameters are supported (`$filter`, `$top`, `$skip`, `$count`, `$orderBy`, `$expand`, `$select`).|
