...
- The application that calls **completeMigration** must be the same application that initiated the migration session by calling [startMigration](channel-startmigration.md) on the target channel. This is the same application which is allowed to call [importMessage](channel-post-messages.md#import-message) during the migration session.
